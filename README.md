...
To enable range test on a remote node, send 'ADRT on _shortname_' in a broadcast channel (ie. ADRT on WP03) -- capitalization is used for emphasis only. The <shortname> is the target node to enable range test. To disable range test, send 'ADRT off' in a broadcast channel - all nodes that receive this will turn off range test. These are examples of `admin` commands that are explained in more depth later in this document.
